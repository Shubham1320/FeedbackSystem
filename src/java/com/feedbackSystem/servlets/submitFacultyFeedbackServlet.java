...
@WebServlet(urlPatterns = {"/submitFacultyFeedback"})
public class submitFacultyFeedbackServlet extends HttpServlet
{
			...
			throws ServletException,IOException
	{
		...
		for(int i=0 ; i < Integer.parseInt(request.getParameter("count")) ; i++) {
			...
			try {
					...
					try {
						 DBUtils.insertIntoFeedbackQuestionaryRelation(conn,feedbackId,questionId,opt1,opt2,opt3,opt4);
            ...
            = this.getServletContext().getRequestDispatcher("/fetchStudentLoadMapping");
